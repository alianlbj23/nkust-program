...
3. **下載相關模組**  
    ...
    :warning:<font color="red">注意!:輸入這段程式後，可能會出現錯誤，那是utf-8的問題，解決方法如下:</font>
...
9. :warning:<font color="red">**注意事項**</font>  
    - 若照著以上步驟還是出現錯誤的話，可以去查看/var/www/apache2/log裡面的erro檔，將最底下的錯誤訊息複製到網路上，通常會有解。
    - 資料夾名稱能不用中文就不用中文，在windows的環境下執行沒問題，但在LINUX系統底下，中文會出一堆編碼的問題。
...
- ### 可以在哪裡找到該程式，以及該如何製作?  
    ...
    `def signup(request):`
    收到前端回傳回來的註冊表單後，先用所有資料欄位對照，看資料庫是否有重複的資料，沒有的話就寫進資料庫。
    登入畫面可在底下程式碼找到:
    `def login(request):`
    只需將所有用戶的資料讀出來傳去前端。
    
  <img src="https://media.giphy.com/media/H1nQWclD7hYsUZbBgg/giphy.gif" width="800" height="400" />
### 短期記憶  
- ### 遊玩方式  
    本遊戲一開始會出現**三張不同物種或可明顯區分類別**的圖片出現五秒，供高齡者記憶，然後出現**五張圖片**，一樣是不同物種或可明顯區分類別的圖片，高齡者要找出一開始出現的那三張圖片。

<img src="https://media.giphy.com/media/7vjQJYdbEv4RxW7fGw/giphy.gif" width="800" height="400" />
...
<img src="https://media3.giphy.com/media/QNERxTWa8mC7qty1kt/giphy.gif?cid=790b7611061c55aeb806d056d361a6adc5e40085d1c129fc&rid=giphy.gif&ct=.gif" width="800" height="400" />
- ### 可以在哪裡找到該程式，以及該如何製作?  
    在views.py底下可找到該部分的實作，可先從以下部份開始看起
    `def OrientationGame(request, pk, n, gameName):`
    `def OrientationPadGame(request, pk, n, gameName):`
    因為有區分電腦版和行動裝置版，因此區分兩個，裡面的邏輯都大同小異，都是先將色塊的顏色決定好後，將一組打亂的傳往前端，前端互動完後再去後端
...
<img src="https://media2.giphy.com/media/K97K1wUcbmfZKXRld8/giphy.gif" width="800" height="400" />
- ### 可以在哪裡找到該程式，以及該如何製作?  
    在views.py底下可找到該部分的實作，可先從以下部份開始看起
    `def AttentionGame(request, pk, n, gameName):`
    基本上後端什麼都不用做，只需等前端互動完後回傳
...
<img src="https://media.giphy.com/media/qen6LRiuu1CNlJO8wI/giphy.gif" width="800" height="400" />
- ### 可以在哪裡找到該程式，以及該如何製作?  
    尚未製作...